...
import { CrudOptions, EntitiesBulk, FilterParamParsed, ObjectLiteral } from "../interfaces";
...
function getOneBaseInit(target: object, name: string, dto: any, crudOptions: CrudOptions) {
    ...
    const pipes = crudOptions.options.idType === "string" ? [] : [setParseIntPipe()];

    setParams(
        {
            ...createParamMetadata(RouteParamtypes.PARAM, 0, pipes, "id"),
...
function createOneBaseInit(
    ...
    if (
        crudOptions.interceptors &&
        crudOptions.interceptors.create &&
        crudOptions.interceptors.create.length
    ) {
        setInterceptors(crudOptions.interceptors.create, prototype[name]);
    }
    if (
        crudOptions.guards &&
        ...
        setGuards(crudOptions.guards.create, prototype[name]);
    }
...
function updateOneBaseInit(
    ...
    prototype[name] = function updateOneBase(id: string, params: ObjectLiteral, body: any) {
        const paramsFilter = this.getParamsFilter(params);
...
function getParamsFilterInit(prototype: any, crudOptions: CrudOptions) {
    prototype.getParamsFilter = function getParamsFilter(
        ...
        return (isArray ? crudOptions.params : Object.keys(crudOptions.params))
            .filter((field: any) => !!params[field])
            .map(
                (field: any) =>
                    ({
                        field: isArray ? field : (crudOptions.params as any)[field],
                        operator: "eq",
                        value: params[field],
